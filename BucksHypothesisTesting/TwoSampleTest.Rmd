...
- **Null Hypothesis:** season ticket members live the same distance or farther from the arena compared to non-season ticket members
- **Alternative Hypothesis:** season ticket members live closer to the arena on average than non-season ticket members
ticket holders

```{r}

# running the t-test on the 2023 dataset
run_t_test(ALGLSL_2023, "DistanceToArena", "STM", "less")

# running the t-test on the 2024 dataset
run_t_test(ALGLSL_2024, "DistanceToArena", "STM", "less")
...
- **Null Hypothesis:** both season and non-season ticket members have the same average interest or greater in Bucks games
...
# running the t-test on the 2023 dataset
run_t_test(ALGLSL_2023, "BasketballPropensity", "STM", "greater")

# running the t-test on the 2023 dataset
run_t_test(ALGLSL_2024, "BasketballPropensity", "STM", "greater")
...
- **Null Hypothesis:** both season and non-season ticket members attend the same average or fewer of Bucks games
- **Alternative Hypothesis:** season ticket members have a higher average of games attended than non-season ticket
members

```{r}

# running the t-test on the 2023 dataset
run_t_test(ALGLSL_2023, "GamesAttended", "STM", "greater")

# running the t-test on the 2023 dataset
run_t_test(ALGLSL_2024, "GamesAttended", "STM", "greater")


```

## Two Sample T-test on GiveawayLabel vs BasketballPropensity
Next, we would like to perform a t-test on comparing the GiveawayLabel against the BasketballPropensity feature. 
Hence, we set the null and alternative hypothesis as follows:

- **Null Hypothesis:** the average BasketballPropensity is the same for users who attended giveaway games and those
 who did not.
- **Alternative Hypothesis:** the average BasketballPropensity differs between users who attended giveaway games 
and those who did not.
 
```{r}

# running the t-test on the 2023 dataset
run_t_test(ALGLSL_2023, "GamesAttended", "STM", "two.sided")

# running the t-test on the 2023 dataset
run_t_test(ALGLSL_2024, "GamesAttended", "STM", "two.sided")
