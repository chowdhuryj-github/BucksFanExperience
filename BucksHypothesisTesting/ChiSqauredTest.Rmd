...
run_chi_squared_test <- function(dataset, feature_one, feature_two) {
    ...
    if(chi_results$p.value < 0.05) {
        ...
        # number of successes
        x <- contingency_table[2,]

        # the total amount
        n <- colSums(contingency_table)

        # post-hoc pairwise test
        p <- pairwise.prop.test(x, n, p.adjust.method = "bonferroni")
        print(p)
...
After establishing the function, we now decide to conduct a chi-squared test on STM against the FanSegment feature. 
We also perform a post-hoc pairwise test.

```{r}

# running the chi-squared test function for the 2023 dataset
run_chi_squared_test(account_2023, 'STM', 'FanSegment')

# running the chi-squared test function for the 2024 dataset
run_chi_squared_test(account_2024, 'STM', 'FanSegment')
...
Now, we decide to create heat maps using the function we created. This will allow us to visualize the
